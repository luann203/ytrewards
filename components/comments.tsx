...
export default function Comments() {
  ...
  return (
    <div className="flex flex-col bg-[#181818] rounded-xl border border-[#303030] mt-3">
      <div className="flex items-center justify-between text-[#aaaaaa] text-sm border-b border-[#303030] p-4">
        <div className="inline-flex items-center justify-center gap-1.5">
          <svg
            aria-hidden="true"
            className="w-5 h-5 fill-current text-[#8AB4F8]"
            viewBox="0 0 448 512"
          >
        ...
        <div className="text-[#aaaaaa]">{`${currentIndex + 1} of ${CommentsData.length} comments`}</div>
      </div>
      ...
      <div className={`p-4 ${progress < 97 && 'appear'}`}>
        <div className="flex items-start gap-3 mb-4">
          ...
          />
          <div>
            <div className="text-sm font-semibold text-[#8AB4F8]">{comment.name}</div>
            <div className="text-xs text-[#aaaaaa]">{comment.time}</div>
            <div className="text-sm mt-1 text-[#f1f1f1]">{comment.content}</div>
          </div>
        </div>
      </div>
    </div>
